...
{
	size_t n = strlen(p) + 1;
	this->m_pStr = new char[n];
	strcpy(m_pStr, p);
	///////////////////////////////Добавляем элементы перед pHead
	this->pNext = pHead;
	this->m_nOwners = 1;
	pHead = this;
		
	///////////////////////////////Добавляем элементы после pHead
	/*
	pNext = nullptr;//pHead;
	...
	//для корректного значения pNext
	Counter* pElement = pHead;
	for (int i = 1; i < Counter::m_curCounters; i++){
